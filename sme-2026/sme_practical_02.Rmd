...
- Rates instead of just counts (accounting for person-time)
...
# Question 2: Mortality Rate by Employment Grade

Do high-grade and low-grade employees have different mortality rates? 

First, let's understand the exposure variable:
...
# Question 3: Is Age a Confounder?
...
First, let's create age groups so we can stratify the analysis:
...
While age is continuous, creating categories makes it easier to see patterns and present results in tables - at the cost of losing some information. The `cut()` function divides the continuous age variable into groups. `right = FALSE` means 40-49 includes 40 but not 50 (50 goes in the next group).
...
The programming approach to do this uses a "split-apply-combine" strategy: we split the data by age 
...
# Question 4: Effect Modification
...
This requires some new code. We use comparisons() within the marginaleffects package to make comparisons. We first tell comparisons() which model to use (the interaction model), which variables to look at (grade), and which group we want (agecat), as well as to report a ratio (instead of e.g. a difference)
...
comparisons(
  ...
  knitr::kable(
    digits = 2,
    col.names = c("Age Group", "Rate Ratio", "95% CI Lower", "95% CI Upper"),
    caption = "Grade Rate Ratios (Low vs High) by Age Group")
```
...
A few final comments. In this practical you have:

1. Calculated rates accounting for person-time (Method 1 by hand, Method 2 with Poisson regression)
2. Compared rates between groups (grade-mortality association)
3. Examine confounding by age 
4. Tested for effect modification (does the association differ by age?)

Relating this back to the real world...

- If you found that low grade employees have higher mortality, what might explain this? (Hint: Think about occupational hazards, stress, access to healthcare...)
- If age is a confounder, what does that tell you about the social patterning of health?
- If there's were to be effect modification, what would it mean that grade matters more (or less) in certain age groups?

A few final reinforcements:

- Confounding is about the structure of the real world, not a property of your data
- P-values tell us about strength of evidence, not about whether something is "true" or "false"
- Always think about biological/social plausibility alongside statistical evidence